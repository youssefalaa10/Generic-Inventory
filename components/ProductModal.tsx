...
import { PlusIcon, TrashIcon, UploadIcon, BarcodeIcon, SparklesIcon } from './Icon';
...
const ProductModal: React.FC<ProductModalProps> = ({ product, allProducts, suppliers, onClose, onSave }) => {
    ...
    const generateSku = () => {
        if (editableProduct.supplierId) {
            const supplier = suppliers.find(s => s.id === editableProduct.supplierId);
            if (supplier?.code) {
                const currentSku = editableProduct.sku || '';
                const baseSku = currentSku.includes('-') ? currentSku.substring(currentSku.indexOf('-') + 1) : currentSku;
                setEditableProduct(prev => ({ ...prev, sku: `${supplier.code}-${baseSku}` }));
                addToast('تم إنشاء SKU بناءً على المورد.', 'info');
            } else {
                addToast('هذا المورد ليس لديه كود.', 'info');
            }
        } else {
            addToast('يرجى اختيار مورد أولاً.', 'error');
        }
    };
                             ...
                             <div className="form-section">
                                ...
                                <div className="form-section-body">
                                    <div style={{marginBottom: '1rem'}}>
                                        <FormField label="المورد الأساسي">
                                            <select
                                                value={editableProduct.supplierId || ''}
                                    ...
                                    <div className="form-grid-cols-2">
                                        <FormField label="الرقم التسلسلي SKU" required>
                                            <div style={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                                                <input type="text" value={editableProduct.sku || ''} onChange={e => handleChange('sku', e.target.value)} className="form-input" style={{flex: 1}} />
                                                <button type="button" onClick={generateSku} className="btn btn-ghost" title="إنشاء SKU بناءً على كود المورد" style={{padding: '0.5rem'}}>
