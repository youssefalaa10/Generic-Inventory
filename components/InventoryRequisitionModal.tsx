...
const InventoryRequisitionModal: React.FC<InventoryRequisitionModalProps> = ({ onClose, onSave, products, branches }) => {
    ...
    const handleAddItem = () => {
        setRequisition(prev => ({ ...prev, items: [...(prev.items || []), { productId: 0, quantity: 1 }] }));
    };
    ...
    const handleSaveClick = () => {
        if (!requisition.warehouseId || (requisition.items || []).length === 0) {
            alert('Please select a warehouse and add at least one item.');
            return;
        }
        onSave(requisition as InventoryRequisition);
    };
                ...
                <div className="modal-body">
                    <div className="form-section">
                        ...
                        <div className="form-section-body" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '1rem' }}>
                            ...
                            <div><label className="form-label">التاريخ</label><input type="date" value={requisition.date} onChange={e => handleFieldChange('date', e.target.value)} className="form-input" /></div>
                            <div>
                                <label className="form-label required">النوع</label>
                                <select value={requisition.type} onChange={e => handleFieldChange('type', e.target.value)} className="form-select">
                            ...
                            <div>
                                ...
                                <select value={requisition.warehouseId} onChange={e => handleFieldChange('warehouseId', Number(e.target.value))} className="form-select">
                                ...
                                <table style={{backgroundColor: '#f9fcff'}}>
                                    ...
                                    <tbody>
                                        {(requisition.items || []).map((item, index) => (
                                            <tr key={index}>
                                                <td style={{padding: '0.5rem'}}><select value={item.productId} onChange={e => handleItemChange(index, 'productId', e.target.value)} className="form-select"><option value="">اختر...</option>{products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></td>
                    ...
                    <div className="form-section">
                        ...
                        <div className="form-section-body" style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem'}}>
                           <div>
                                ...
                                <textarea value={requisition.notes || ''} onChange={(e) => handleFieldChange('notes', e.target.value)} className="form-input" rows={5}></textarea>
                           </div>
                           <div>
