...
const Customers: React.FC<CustomersProps> = ({ customers, onSave, whatsappSettings, branches, onCustomerSelect }) => {
                    ...
                    <table>
                        ...
                        <tbody>
                            {filteredCustomers.map(c => (
                                <tr key={c.id} onClick={() => onCustomerSelect(c)} style={{ cursor: 'pointer' }}>
                                    <td>{c.name}</td>
                                    <td>{c.phone}</td>
                                    ...
                                    <td>
                                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                                            <button onClick={(e) => { e.stopPropagation(); handleEdit(c); }} style={{color: '#f59e0b', background: 'none', border: 'none', cursor: 'pointer', padding: '0.25rem'}} title="تعديل"><PencilIcon style={{width:'20px', height:'20px'}}/></button>
                                            {whatsappSettings.isEnabled && c.id !== 4 && (
                                                <button onClick={(e) => { e.stopPropagation(); handleOpenWhatsappModal(c); }} style={{color: '#10b981', background: 'none', border: 'none', cursor: 'pointer', padding: '0.25rem'}} title="إرسال رسالة واتساب"><ChatIcon style={{width:'20px', height:'20px'}}/></button>
                                            )}
                                        </div>
                                    </td>
                                </tr>
