...
import { ProductionTask, ManufacturingOrder, EmployeeData, ProductionTaskStatus } from '../types';
...
const ProductionTasks: React.FC<ProductionTasksProps> = ({ tasks, orders, employees, onSave }) => {
    ...
    return (
        <>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', height: 'calc(100vh - 110px)' }}>
                <div className="glass-pane" style={{ padding: '1rem 1.5rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <h3 style={{ fontSize: '1.25rem', fontWeight: 600 }}>لوحة مهام التصنيع</h3>
                    <button onClick={handleAddNew} className="btn btn-primary">
                        + مهمة جديدة
                    </button>
                </div>

                <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', overflowX: 'auto' }}>
                    {statusColumns.map(status => (
                        <div key={status} onDrop={(e) => handleDrop(e, status)} onDragOver={allowDrop}
                             className="glass-pane" style={{ padding: '1rem', display: 'flex', flexDirection: 'column' }}>
                            <h4 style={{ padding: '0.5rem', borderRadius: '8px', textAlign: 'center', fontWeight: 600, color: 'var(--text-primary)', borderBottom: '1px solid var(--surface-border)', marginBottom: '1rem' }}>
                            ...
                            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem', padding: '0 0.5rem' }}>
                                {tasks.filter(t => t.status === status).map(task => (
                                    <div key={task.id} draggable onDragStart={(e) => handleDragStart(e, task.id)}
                                         className="glass-pane" style={{ padding: '1rem', cursor: 'grab', display: 'flex', flexDirection: 'column' }}>
                                        <div style={{ flex: 1 }}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                <p style={{ fontWeight: 'bold', flex: 1, paddingLeft: '0.5rem' }}>{task.name}</p>
                                            ...
                                            {task.deadline && (
                                                <p style={{ fontSize: '0.8rem', color: new Date(task.deadline) < new Date() ? '#ef4444' : 'var(--text-secondary)', fontWeight: 600, marginTop: '0.5rem' }}>
                                                    الموعد النهائي: {task.deadline}
                                                </p>
                                            )}
                                        </div>

                                        <div style={{ borderTop: '1px solid var(--surface-border)', marginTop: '0.75rem', paddingTop: '0.75rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
