...
import { IntegrationSettings, EcommerceIntegrationSettings, PaymentGatewaySettings, WhatsAppSettings, N8nSettings, Webhook, AISettings } from '../types';
...
const IntegrationsPage: React.FC<IntegrationsPageProps> = ({ settings, onSave }) => {
    ...
    const handleAiChange = (newSettings: AISettings) => {
        setLocalSettings(prev => ({ ...prev, ai: newSettings }));
    };
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                ...
                <AICard title="إعدادات الذكاء الاصطناعي" settings={localSettings.ai} onChange={handleAiChange} />
            </div>
...
const AICard: React.FC<{ title: string; settings: AISettings; onChange: (s: AISettings) => void; }> = ({ title, settings, onChange }) => {
    const handleProviderChange = (provider: 'Gemini') => {
    ...
    return (
        <IntegrationCard title={title} isEnabled={settings.isEnabled} onToggle={(val) => onChange({ ...settings, isEnabled: val })}>
            <div style={{display: 'flex', flexDirection: 'column', gap: '1rem'}}>
                <FormField label="مزود الخدمة الحالي">
                    <select
                        className="form-select"
                        value={settings.provider}
                        onChange={e => handleProviderChange(e.target.value as 'Gemini')}
                        disabled // Disabled for now as only Gemini is supported.
                    >
                ...
                <div style={{display: 'flex', gap: '1rem', alignItems: 'center'}}>
                    <SparklesIcon style={{width: '48px', height: '48px', color: 'var(--primary-color)', flexShrink: 0}}/>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: 1.6 }}>
                        تفعيل ميزات الذكاء الاصطناعي في النظام مثل المسح الضوئي للفواتير، اقتراحات أوامر الشراء، والمساعد الذكي "فهيم".
                        <br/>
                        <strong>ملاحظة:</strong> يجب تكوين مفتاح API لمزود الخدمة المختار في متغيرات البيئة الخاصة بالخادم (<code>API_KEY</code>) لتعمل هذه الميزات.
                    </p>
                </div>
            </div>
        </IntegrationCard>
