...
import { User, Role, Permission, Project, Branch, PurchaseInvoice, Sale, EmployeeData, RenewableItem, Product, InventoryItem, LeaveRequest, AttendanceRecord, Customer, FinancialAccount, Expense, POSSession, ProductionOrder_Legacy, InventoryAdjustmentLog, ProductionTask, ManufacturingOrder, Account, IntegrationSettings, WebhookEvent, AdvanceRequest, GeneralRequest, Supplier, PurchaseRequest, PurchaseOrder, PurchaseReturn, SupplierPayment, DebitNote, RequestForQuotation, PurchaseQuotation, ExpenseCategory, SalesQuotation, SalesReturn, CreditNote, CustomerPayment, RecurringInvoice, InventoryVoucher, InventoryRequisition, PurchaseSettings, JournalVoucher, WhatsappLog } from '../types';
import { PERMISSIONS } from '../constants';

export const USERS: User[] = [
    { id: 1, name: 'ضرار حرارة', role: Role.SuperAdmin, permissions: PERMISSIONS[Role.SuperAdmin] },
    { id: 2, name: 'ضرار حرارة (صانع عطور)', role: Role.Perfumer, permissions: PERMISSIONS[Role.Perfumer], branchId: 4 },
    { id: 3, name: 'سليمان شختور (محاسب)', role: Role.Accountant, permissions: PERMISSIONS[Role.Accountant] },
    { id: 4, name: 'فراس فريد (مدير فرع)', role: Role.BranchManager, permissions: PERMISSIONS[Role.BranchManager], branchId: 1 },
    { id: 5, name: 'فهد (بائع)', role: Role.ShopAssistant, permissions: PERMISSIONS[Role.ShopAssistant], branchId: 1 },
    { id: 6, name: 'وليد شوبري (مدير تسويق)', role: Role.EcommerceManager, permissions: PERMISSIONS[Role.EcommerceManager], branchId: 4 },
    { id: 7, name: 'بلال (موظف)', role: Role.Employee, permissions: PERMISSIONS[Role.Employee], branchId: 3 },
];
...
export const BRANCHES: Branch[] = [
    { id: 1, projectId: 2, name: 'ارابيفا 1 - سوق واجف' },
    { id: 2, projectId: 2, name: 'ارابيفا 2 - شارع الغربللي' },
    { id: 3, projectId: 2, name: 'ارابيفا 3 - كويت ماجك' },
    { id: 4, projectId: 1, name: 'المخازن - مصنع جينيرك الاحمدي' },
    { id: 5, projectId: 1, name: 'المخازن - مخزن الفنطاس' }
];

export const MOCK_SUPPLIERS: Supplier[] = [
    { id: 1, code: '22', name: 'مورد العطور الدولي', contactPerson: 'علي رضا', email: 'ali.reza@international-perfumes.com', phone: '+971 4 123 4567', address: 'دبي, الإمارات العربية المتحدة', balance: 3500 },
    { id: 2, code: '55', name: 'مصنع الزجاجيات المحلي', contactPerson: 'سالم مبارك', email: 'salem@kuwait-glass.com', phone: '+965 2222 3333', address: 'الصليبية, الكويت', balance: 0 },
    { id: 3, code: '77', name: 'مطابع النخبة', contactPerson: 'فاطمة أحمد', email: 'info@elite-print.kw', phone: '+965 1800 123', address: 'الشويخ الصناعية, الكويت', balance: -200 }, // Credit balance
];


export const PRODUCTS: Product[] = [
    // Raw Materials - Oils
    { id: 1, name: 'زيت الورد الدمشقي', sku: 'RM-OIL-001', category: 'Raw Material', unitPrice: 15, baseUnit: 'g', density: 0.98, trackInventory: true, hasExpiryDate: true, supplierId: 1 },
    { id: 2, name: 'زيت العود الكمبودي', sku: 'RM-OIL-002', category: 'Raw Material', unitPrice: 40, baseUnit: 'g', density: 0.95, trackInventory: true, hasExpiryDate: true, supplierId: 1 },
    { id: 3, name: 'زيت المسك الأبيض', sku: 'RM-OIL-003', category: 'Raw Material', unitPrice: 5, baseUnit: 'g', density: 1.05, trackInventory: true, hasExpiryDate: true, supplierId: 1 },
    { id: 102, name: 'زيت العنبر والفانيلا', sku: 'RM-OIL-004', category: 'Raw Material', unitPrice: 8, baseUnit: 'g', density: 1.02, trackInventory: true, hasExpiryDate: true, supplierId: 1 },
    ...
    // Packaging
    { id: 4, name: 'زجاجة عطر 50مل (شفاف)', sku: 'PKG-BTL-001', category: 'Packaging', unitPrice: 2.5, baseUnit: 'pcs', trackInventory: true, supplierId: 2 },
    { id: 5, name: 'زجاجة عطر 100مل (أسود)', sku: 'PKG-BTL-002', category: 'Packaging', unitPrice: 3.5, baseUnit: 'pcs', trackInventory: true, supplierId: 2 },
    { id: 6, name: 'علبة كرتون فاخرة (أسود)', sku: 'PKG-BOX-001', category: 'Packaging', unitPrice: 1, baseUnit: 'pcs', trackInventory: true, supplierId: 3 },
    { id: 201, name: 'بخاخ ذهبي 18مم', sku: 'PKG-SPR-001', category: 'Packaging', unitPrice: 0.25, baseUnit: 'pcs', trackInventory: true, supplierId: 2 },
    { id: 202, name: 'غطاء مغناطيسي أسود', sku: 'PKG-CAP-001', category: 'Packaging', unitPrice: 0.4, baseUnit: 'pcs', trackInventory: true, supplierId: 2 },
    ...
    { 
     ...
     { 
        id: 10, 
        name: 'Arabiva Rose Blush 50ml',
        productLine: "Floral Collection",
        fragranceNotes: { top: 'Bergamot', middle: 'Damascus Rose, Peony', base: 'White Musk' },
        sku: 'ARB-FG-002', 
        category: 'Finished Good', 
        unitPrice: 42,
        baseUnit: 'pcs',
    ...
    // Sets / Stocks for Kuwait Magic
    {
        id: 501,
        name: 'Kuwait Magic Special Set',
        sku: 'SET-KM-001',
        category: 'Set',
        unitPrice: 75, // Special price for the set
        baseUnit: 'pcs',
        trackInventory: true, // Tracked via components
        components: [
            { productId: 8, quantity: 1 }, // 1x Arabiva Oud Royal 50ml
            { productId: 10, quantity: 1 }, // 1x Arabiva Rose Blush 50ml
        ]
    }
];
...
BRANCHES.forEach(branch => {
    PRODUCTS.forEach(product => {
        // Logic to determine if a product should be in a specific branch type
        const isWarehouse = branch.name.includes('المخازن');
        const isRetail = !isWarehouse;

        if ((product.category === 'Raw Material' || product.category === 'Packaging') && isRetail) {
            // Don't add raw materials or packaging to retail branches
            return;
        }
        
        if ((product.category === 'Finished Good' || product.category === 'Set') && isWarehouse) {
            // Only add a small chance of finished goods being in warehouses, for transfers
        ...
        if (Math.random() > 0.10) { // 10% chance of being out of stock
            if (product.category === 'Raw Material') {
                 if (product.baseUnit === 'g' || product.baseUnit === 'ml') {
                    quantity = Math.floor(Math.random() * 9000) + 1000; // 1,000 to 10,000
                 } else {
                    quantity = Math.floor(Math.random() * 450) + 50; // 50 to 500 for pcs
                 }
            } else if (product.category === 'Packaging') {
                quantity = Math.floor(Math.random() * 4500) + 500; // 500 to 5,000
            } else { // Finished Good or Set
                quantity = Math.floor(Math.random() * 200) + 20; // 20 to 220
            }
        }
        
        const minStock = Math.max(5, Math.floor(quantity / 5)); // Minimum stock is at least 5 or 20%
        ...
        if (product.hasExpiryDate) {
            ...
            expiry.setFullYear(expiry.getFullYear() + Math.floor(Math.random() * 2) + 1); // 1-2 years in the future
            expiry.setMonth(Math.floor(Math.random() * 12));
            newItem.expiryDate = expiry.toISOString().split('T')[0];
        }
...
// A helper function to ensure specific items have enough stock in a specific branch for demo purposes
const ensureStock = (branchId: number, productId: number, requiredQty: number) => {
    const itemIndex = newInventory.findIndex(i => i.branchId === branchId && i.productId === productId);
    if (itemIndex > -1) {
        if (newInventory[itemIndex].quantity < requiredQty) {
            newInventory[itemIndex].quantity = requiredQty + Math.floor(Math.random() * 1000);
        }
    } else {
         newInventory.push({
             branchId: branchId,
             productId: productId,
             quantity: requiredQty + Math.floor(Math.random() * 1000),
             minStock: Math.max(5, requiredQty / 5),
         });
    }
};

// Ensure stock for the mock manufacturing order (MO-20240728-001) in branch 4 (Manufacturing)
ensureStock(4, 102, 1000); // زيت العنبر والفانيلا
ensureStock(4, 103, 200);  // مثبت - مسك كيتون
ensureStock(4, 7, 5000);   // كحول عطور 96%
ensureStock(4, 101, 500);  // ماء مقطر

// Ensure stock for packaging for the same order (assuming it's also in branch 4 for simplicity)
// The manufacturing branch is a warehouse, but we need to ensure packaging materials are there.
ensureStock(4, 4, 200);    // زجاجة عطر 50مل
ensureStock(4, 201, 200);  // بخاخ ذهبي 18مم


export const INVENTORY: InventoryItem[] = newInventory;
...
export const MOCK_PURCHASE_INVOICES: PurchaseInvoice[] = [
    ...
    { 
        id: 2, branchId: 5, brand: 'Generic', supplierId: 2, 
        date: '2024-03-20',
...
export const SALES: Sale[] = [
    { id: 101, branchId: 1, brand: 'Arabiva', invoiceNumber: 'INV-A-001', customerId: 4, customerName: 'زبون نقدي عام', date: '2024-07-22', totalAmount: 90, paymentMethod: 'K-Net', paymentStatus: 'Paid', sessionId: 1, source: 'In-Store', items: [
        { id: 1, productName: 'Arabiva Oud Royal 50ml', productId: 8, quantity: 2, unitPrice: 45, total: 90 },
    ]},
    { id: 102, branchId: 3, brand: 'Arabiva', invoiceNumber: 'INV-A-002', customerId: 1, customerName: 'شركة الأفق للتجارة', date: '2024-07-22', totalAmount: 24, paymentMethod: 'Card', paymentStatus: 'Paid', sessionId: 1, source: 'In-Store', items: [
        { id: 2, productName: 'Generic Pure Musk Oil 12ml', productId: 9, quantity: 2, unitPrice: 12, total: 24 },
    ]},
    { id: 103, branchId: 4, brand: 'Generic', invoiceNumber: 'WEB-G-001', customerId: 2, customerName: 'مؤسسة النور للعطور', date: '2024-07-28', totalAmount: 120, paymentMethod: 'MyFatoorah', paymentStatus: 'Paid', source: 'Website', items: [
        { id: 3, productName: 'Generic Pure Musk Oil 12ml', productId: 9, quantity: 10, unitPrice: 12, total: 120 },
    ]},
];

export const MOCK_WHATSAPP_LOGS: WhatsappLog[] = [
    { id: 1, customerId: 1, date: '2024-07-25T10:00:00Z', message: 'مرحباً، شكراً لشرائكم من أرابيفا. هل يمكننا الحصول على تقييمكم؟', status: 'Read' },
    { id: 2, customerId: 1, date: '2024-07-29T14:30:00Z', message: 'عرض خاص لعملاءنا المميزين: خصم 20% على عطر العود الملكي', status: 'Delivered' },
    { id: 3, customerId: 2, date: '2024-07-28T18:00:00Z', message: 'تم شحن طلبكم رقم WEB-G-001 بنجاح!', status: 'Read' },
];
...
export const MOCK_INVENTORY_VOUCHERS: InventoryVoucher[] = [
    {
        ...
        branch: 'المخازن - مصنع جينيرك الاحمدي',
        type: 'down'
    },
    {
        id: 'VO-2024-002',
...
export const MOCK_INVENTORY_REQUISITIONS: InventoryRequisition[] = [
    ...
    },
    {
        ...
        date: '2024-07-26',
        type: 'Purchase',
        warehouseId: 5, // Fintas Warehouse
        items: [
            { productId: 4, quantity: 2000 }, // 50ml Bottle
        ],
        notes: 'طلب شراء عبوات ناقصة'
    }
];
...
export const FINANCIAL_ACCOUNTS: FinancialAccount[] = [
    ...
    { id: 5, name: 'خزينة فرع سوق واجف', type: 'Cash', branchId: 1, balance: 1500 },
];
