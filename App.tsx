...
const AppContent: React.FC = () => {
    ...
    const [activeView, setActiveView] = useState('Reports/Forecast');
    ...
    const [whatsappLogs, setWhatsappLogs] = useState<WhatsappLog[]>(MOCK_WHATSAPP_LOGS);
    ...
    const handleSaveSale = (sale: Sale) => {
        setSales(prev => {
            ...
            if (exists) {
                // Logic for updating a sale (if needed) can be complex, for now we only handle new sales
            ...
            setInventory(prevInv => {
                ...
                newSale.items.forEach(item => {
                    ...
                    // If the sold item is a "Set", deduct its components from inventory
                    if (productDetails?.category === 'Set' && productDetails.components && productDetails.components.length > 0) {
                        productDetails.components.forEach(component => {
                            ...
                            if (invIndex > -1) {
                                ...
                                console.warn(`Component product ID ${component.productId} not found in inventory for branch ${newSale.branchId} to deduct for set sale.`);
                            }
                        });
                    } else {
                        // For regular products, deduct the product itself
                        const invIndex = newInv.findIndex(i => i.branchId === newSale.branchId && i.productId === item.productId);
                        if (invIndex > -1) {
                            newInv[invIndex].quantity -= item.quantity;
                        } else {
                            console.warn(`Attempted to sell product ID ${item.productId} which has no inventory record in branch ${newSale.branchId}.`);
                        }
                    }
                });
                return newInv;
    ...
    const renderView = () => {
        ...
        if (activeView.startsWith('Purchases/Suppliers')) return <Suppliers suppliers={suppliers} onSave={handleSaveSupplier} purchaseInvoices={purchaseInvoices} />;
        ...
        if (activeView === 'Inventory/Tracking') {
        ...
        if (['Inventory/Stocktakes', 'Inventory/Transfers', 'Inventory/Pricelists'].includes(activeView)) {
             return <div className="glass-pane" style={{padding: '2rem', textAlign: 'center'}}>Coming Soon: {activeView.split('/')[1]}</div>
        }
        
        // Other Modules
        if (activeView.startsWith('POS/Start')) return <POS posView="default" products={products} inventory={inventory} customers={customers} onSaveCustomer={handleSaveCustomer} onSave={handleSaveSale} integrationSettings={integrationSettings} branches={branches} />;
        if (activeView.startsWith('POS/KuwaitMagic')) return <POS posView="kuwaitMagic" products={products} inventory={inventory} customers={customers} onSaveCustomer={handleSaveCustomer} onSave={handleSaveSale} integrationSettings={integrationSettings} branches={branches} />;
        ...
        if (activeView.startsWith('Customers/')) {
            const customerId = parseInt(activeView.split('/')[2], 10);
        ...
        if (activeView.startsWith('Customers')) return <Customers customers={customers} onSave={handleSaveCustomer} whatsappSettings={integrationSettings.whatsapp} branches={branches} onCustomerSelect={(customer) => setActiveView(`Customers/${customer.id}`)} />;
